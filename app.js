var express = require('express'),
    ...
    app.get('/search',(req,res)=>{
        ...
        //let surl = "https://www.mangazuki.online/?s="+req.body.name+"&post_type=wp-manga"        
        axios.get(surl).then(response=>{
            let $ = cheerio.load(response.data);
            $('.h4').children().each(function(i,manga){
                mangaSearchList[i] = $(manga).text().toString().replace(/\s+/g, '-').toLowerCase();
                mangaSearch[i]   = {title : $(manga).text() , name : mangaSearchList[i], url : $(manga).attr('href')};
                //console.log(mangaSearch[i]);
        });
    });
    res.render("search.ejs",{mangas:mangaSearch});
});
